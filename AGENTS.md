...
`objectsize` calculates a Go variable's (or object tree's) memory consumption at runtime, building on `binary.Size()` from the standard library. Originally forked from `slytomcat/size`, it has since been extended and is actively maintained as its own project.
